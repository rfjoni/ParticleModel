...
class LactoseModel:
    ...
    def dxdt(self, x, t, z):
        ...
        S = self.solute.supersaturation(C=C, T=T)
        mc = self.solute.mass(N=N, domain=self.domain) # g lactose/µL

        # Calculate phenomena kinetics
        kinetics = self.kinetics.rates(S=S, mc=mc, T=T)
        ...
        # Calculate dCdt
        dCdt = -self.solute.mass(N=dNdt, domain=self.domain)
